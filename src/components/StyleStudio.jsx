...
// The plain uid() helper above is fine for comment ids (stored as text) but
// NOT valid for uuid columns, which caused "Add to closet" to silently fail.
function newUUID() {
...
export default function StyleStudio() {
  ...
  async function boot() {
    try {
      const ik = await storage.list('item:');
      const loaded = [];
      if (ik?.keys) for (const k of ik.keys) { try { const r = await storage.get(k); if (r?.value) loaded.push(JSON.parse(r.value)); } catch(e){} }
      loaded.sort((a,b) => b.dateAdded - a.dateAdded);
      setItems(loaded);

      const ok = await storage.list('outfit:');
      const sl = [];
      if (ok?.keys) for (const k of ok.keys) { try { const r = await storage.get(k); if (r?.value) sl.push(JSON.parse(r.value)); } catch(e){} }
      sl.sort((a,b) => b.dateSaved - a.dateSaved);
      setSavedOutfits(sl);
      setSavedIds(new Set(sl.map(o => o.signature)));

      try { const g = await storage.get('gender_pref'); if (g?.value) setGenderPref(g.value); } catch(e){}
      try { const rj = await storage.get('rejected_outfits'); if (rj?.value) setRejectedOutfits(new Set(JSON.parse(rj.value))); } catch(e){}

      let uid2;
      try { const u = await storage.get('user_id'); uid2 = u?.value; } catch(e){}
      if (!uid2) { uid2 = uid('u_'); try { await storage.set('user_id', uid2); } catch(e){} }
      setUserId(uid2);

      try { const n = await storage.get('display_name'); if (n?.value) setDisplayName(n.value); } catch(e){}
      try { const b = await storage.get('profile_bio'); if (b?.value) setProfileBio(b.value); } catch(e){}
      try { const ph = await storage.get('profile_photo'); if (ph?.value) setProfilePhoto(ph.value); } catch(e){}
      try { const dp = await storage.get('default_public'); if (dp?.value != null) setDefaultPublic(dp.value === 'true' || dp.value === true); } catch(e){}
      try { const sb = await storage.get('saved_board'); if (sb?.value) setSavedBoard(new Set(JSON.parse(sb.value))); } catch(e){}
      try { const l = await storage.get('liked_posts'); if (l?.value) setLikedPosts(new Set(JSON.parse(l.value))); } catch(e){}
  ...
  async function loadFeed() {
    setLoadingFeed(true);
    try {
      const keys = await storage.list('post:', true);
      const loaded = [];
      if (keys?.keys) for (const k of keys.keys) { try { const r = await storage.get(k, true); if (r?.value) loaded.push(JSON.parse(r.value)); } catch(e){} }
  ...
  async function saveGenderPref(g) { setGenderPref(g); try { await storage.set('gender_pref', g); } catch(e){} }

  async function saveItem(item) { await storage.set(`item:${item.id}`, JSON.stringify(item)); setItems(prev => [item, ...prev]); }

  async function deleteItem(id) {
    try { await storage.delete(`item:${id}`); setItems(prev => prev.filter(i => i.id !== id)); showToast('Removed from closet'); } catch(e){}
  }
  ...
  async function incrementWear(itemIds) {
    ...
    for (const u of updates) {
      try { await storage.set(`item:${u.id}`, JSON.stringify(u)); } catch(e){}
    }
  ...
  async function toggleSave(outfit, occasion) {
    ...
    if (savedIds.has(sig)) {
      ...
      if (ex) { try { await storage.delete(`outfit:${ex.id}`); setSavedOutfits(prev => prev.filter(o => o.id !== ex.id)); setSavedIds(prev => { const n=new Set(prev); n.delete(sig); return n; }); showToast('Removed'); } catch(e){} }
    } else {
      const id = newUUID(); const om = OCCASIONS.find(o => o.id === occasion);
      ...
      try { await storage.set(`outfit:${id}`, JSON.stringify(saved)); setSavedOutfits(prev => [saved,...prev]); setSavedIds(prev => new Set(prev).add(sig)); showToast('Saved to looks 🌿'); } catch(e){}
    }
  }

  async function deleteSaved(id) {
    const out = savedOutfits.find(o => o.id === id);
    try { await storage.delete(`outfit:${id}`); setSavedOutfits(prev => prev.filter(o => o.id !== id)); if (out) setSavedIds(prev => { const n=new Set(prev); n.delete(out.signature); return n; }); showToast('Removed'); } catch(e){}
  }
  ...
  async function rejectOutfit(outfit, occasion) {
    ...
    try { await storage.set('rejected_outfits', JSON.stringify([...next])); } catch(e){}
  ...
  async function saveDisplayName(name) {
    ...
    setDisplayName(t); try { await storage.set('display_name', t); } catch(e){}
    setShowNameSetup(false);
  }

  async function saveProfile({ name, bio, photo, isPublic }) {
    if (name != null) { const t = name.trim().slice(0,24); setDisplayName(t); try { await storage.set('display_name', t); } catch(e){} }
    if (bio != null) { const b = bio.slice(0,160); setProfileBio(b); try { await storage.set('profile_bio', b); } catch(e){} }
    if (photo != null) { setProfilePhoto(photo); try { await storage.set('profile_photo', photo); } catch(e){} }
    if (isPublic != null) { setDefaultPublic(isPublic); try { await storage.set('default_public', String(isPublic)); } catch(e){} }
  ...
  async function toggleBoard(post) {
    ...
    setSavedBoard(next);
    try { await storage.set('saved_board', JSON.stringify([...next])); } catch(e){}
  }
  ...
  async function deleteAccount() {
    ...
    try {
      ...
      for (const prefix of ['item:', 'outfit:', 'post:', 'cmt:']) {
        const r = await storage.list(prefix).catch(()=>null);
        if (r?.keys) allKeys.push(...r.keys);
      }
      for (const k of allKeys) { try { await storage.delete(k); } catch(e){} }
      for (const single of ['display_name','profile_bio','profile_photo','default_public','saved_board','gender_pref','rejected_outfits','liked_posts']) {
        try { await storage.delete(single); } catch(e){}
      }
  ...
  async function deletePost(pid) {
    try { await storage.delete(`post:${pid}`, true); setPosts(prev => prev.filter(p => p.id !== pid)); setSelectedPost(null); showToast('Removed'); } catch(e){}
  }

  async function toggleLike(post) {
    if (!userId) return;
    const wasLiked = likedPosts.has(post.id);
    // Optimistic UI flip for the heart icon
    const nl = new Set(likedPosts);
    if (wasLiked) nl.delete(post.id); else nl.add(post.id);
    setLikedPosts(nl);
    try {
      const newCount = await togglePostLike(post.id);
      setPosts(prev => prev.map(p => p.id === post.id ? {...p, likes: newCount} : p));
      if (selectedPost?.id === post.id) setSelectedPost(prev => ({...prev, likes: newCount}));
    } catch (e) {
  ...
  async function loadComments(postId) {
    ...
    try {
      const r = await storage.list(`cmt:${postId}_`, true);
      const loaded = [];
      if (r?.keys) for (const k of r.keys) {
        try { const v = await storage.get(k, true); if (v?.value) loaded.push(JSON.parse(v.value)); } catch(e){}
      }
  ...
  async function addComment(post, text) {
    ...
    try {
      ...
      await storage.set(`cmt:${post.id}_${cid}`, JSON.stringify(comment), true);
  ...
  async function deleteComment(post, commentId) {
    try {
      await storage.delete(`cmt:${post.id}_${commentId}`, true);
  ...
  async function generatePackingList(trip) {
    ...
    try {
      ...
      if (!parsed || parsed.error) throw new Error(parsed?.error || 'Could not read response');
  ...
  async function analyzeClothing(dataUrl) {
    ...
    const parsed = await response.json();
    if (!parsed || parsed.error) throw new Error(parsed?.error || 'Could not read AI response');
  ...
  async function confirmAdd() {
    ...
    const item = {id: newUUID(), dataUrl: pendingItem.dataUrl, ...pendingItem.analysis, dateAdded: Date.now()};
    try {
      await saveItem(item);
      setPendingItem(null); setView(null); showToast('Added to closet 🌿');
    } catch (err) {
      console.error('confirmAdd failed:', err);
  ...
  async function generateOutfits(occasion) {
    ...
    const avoid = rejectedForOccasion.length > 0 ? rejectedForOccasion.join('\n') : '';
    try {
      const response = await fetch('/api/outfits', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ occasion, genderPref, wardrobe, avoid })
      });
      if (!response.ok) throw new Error(`API ${response.status}`);
      const parsed = await response.json();
